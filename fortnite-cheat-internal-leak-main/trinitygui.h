...
namespace Trinity
{
	...
	bool Button(char* name, FVector2D size)
	{
		...
		if (isHovered)
		{
			drawFilledRectA(FVector2D{ pos.X, pos.Y }, size.X, size.Y, FLinearColor{ 0.501960814f, 0.501960814f, 0.501960814f, 1.000000000f });
			hover_element = true;
		}
		else
		{
			drawFilledRectA(FVector2D{ pos.X, pos.Y }, size.X, size.Y, FLinearColor{ 0.662745118f, 0.662745118f, 0.662745118f, 1.000000000f });
		}
	...
	void Hotkey(char* name, FVector2D size, int* key)
	{
		...
		if (active_hotkey == elements_count)
		{
			...
			if (!Trinity::Input::IsAnyMouseDown())
			{
	...
	void Render()
	{
		for (int i = 0; i < 128; i++)
		{
			if (PostRenderer::drawlist[i].type != -1)
			{
				...
				if (PostRenderer::drawlist[i].type == 1)
				{
					Trinity::drawFilledRectA(PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].size.X, PostRenderer::drawlist[i].size.Y, PostRenderer::drawlist[i].color);
				}
				//TextLeft
				else if (PostRenderer::drawlist[i].type == 2)
				{
					Trinity::TextLeft(PostRenderer::drawlist[i].name, PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].color, PostRenderer::drawlist[i].outline);
				}
				//TextCenter
				else if (PostRenderer::drawlist[i].type == 3)
				{
					Trinity::TextCenter(PostRenderer::drawlist[i].name, PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].color, PostRenderer::drawlist[i].outline);
				}
				//Draw_Line
